...
class neural_implict_interploation():
    def __init__(self, data, ratio, sample_count=None):
        self.data = data
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.model = NIII().to(self.device)

        if sample_count is not None:
            self.checkpoint = rf'./model_parameters/NIII_parameter_L{ratio}_{sample_count}samples.pth.tar'
        else:
            self.checkpoint = rf'./model_parameters/NIII_parameter_L{ratio}.pth.tar'

        self.checkpoint_dict = torch.load(self.checkpoint, map_location= self.device, weights_only=False)
...
@timer
def NIII_process(nii_data, ratio, sample_count=None):
    result = neural_implict_interploation(nii_data, ratio, sample_count).inference()
    return result    