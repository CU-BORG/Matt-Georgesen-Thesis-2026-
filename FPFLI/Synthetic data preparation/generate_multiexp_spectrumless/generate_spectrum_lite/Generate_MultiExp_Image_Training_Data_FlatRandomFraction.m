%% Generate Multi-Exponential Training Data (FLAT RANDOM FRACTION)
% Spatially-correlated tau with uniform random fractions [0.05-0.95]
% @author: mg
...
% Spatial correlation parameters (for tau maps only)
config.spatial_correlation_length = 10;  % pixels (biological structure size)
config.correlation_method = 'intensity-guided';  % 'gaussian' or 'intensity-guided'
config.intensity_modulation = 0.3;  % 0-1, strength of intensity effect on tau

% --- FLAT RANDOM FRACTION parameters (the key difference) ---
% Range that f_component1 (bound/short-tau fraction) spans.
% Each pixel is drawn uniformly from [F_MIN, F_MAX] with NO spatial correlation.
config.f_range = [0.05, 0.95];

fprintf('=================================================================\n');
fprintf('Multi-Exponential Image Training Data Generation\n');
fprintf('(SPATIAL CORR + FLAT RANDOM FRACTION)\n');
fprintf('=================================================================\n');
...
fprintf('\nFlat Random Fraction Settings:\n');
fprintf('  Fraction range: [%.2f, %.2f]\n', config.f_range(1), config.f_range(2));
fprintf('  Distribution: UNIFORM RANDOM (no spatial correlation)\n');
fprintf('  Each pixel independently drawn from Uniform[F_MIN, F_MAX]\n');
fprintf('-----------------------------------------------------------------\n');
...
for std_level = 1:STD_LEVELS
    ...
    config.output_dir = sprintf('flatrandom_level_%02d_std_%.2f_%.2f', ...
    ...
    fprintf('-----------------------------------------------------------------\n');
    fprintf('LEVEL %d/%d: Tau1_std=%.2f ns, Tau2_std=%.2f ns (FLAT RANDOM)\n', ...
...
for img_idx = 1:config.N_images
    ...
    %% Normalize intensity for modulation (used by tau maps)
    intensity_normalized = (img_256 - min(img_256(:))) / (max(img_256(:)) - min(img_256(:)) + 1e-10);

    %% Generate SPATIALLY-CORRELATED tau maps (same logic as before)
    ...
    output_filename = sprintf('Sample_%03d_flatrandom.mat', img_idx);
...
first_file = fullfile(config.output_dir, 'Sample_001_flatrandom.mat');
...
ylabel('Count');
title('Bound Fraction Distribution (should be UNIFORM)');
grid on;
...
% Intensity vs bound fraction — should show NO correlation
subplot(3, 4, 11);
...
title('Intensity vs Bound Fraction (should be uncorrelated)');
grid on;
...
for i = 1:length(sample_indices)
    sample_file = fullfile(config.output_dir, sprintf('Sample_%03d_flatrandom.mat', sample_indices(i)));
...
sgtitle(sprintf('Level %d FLAT RANDOM (tau1=%.2f+/-%.3f, tau2=%.2f+/-%.3f ns, f1=[%.2f,%.2f] uniform)', ...
...
fig_name = fullfile(config.output_dir, sprintf('flatrandom_level_%02d_visualization.png', std_level));
try
    saveas(gcf, fig_name);
...
fprintf('\n=================================================================\n');
fprintf('DATA GENERATION COMPLETE (FLAT RANDOM FRACTION)!\n');
fprintf('=================================================================\n');
fprintf('Generated %d standard deviation levels with FLAT RANDOM FRACTION:\n', STD_LEVELS);
fprintf('  - Tau centers: %.2f ns, %.2f ns\n', TAU_1_CENTER, TAU_2_CENTER);
...
fprintf('  - Fraction range: [%.2f, %.2f] (uniform random, no spatial correlation)\n', config.f_range(1), config.f_range(2));
...
for i = 1:STD_LEVELS
    dir_name = sprintf('  flatrandom_level_%02d_std_%.2f_%.2f/', i, TAU_1_STD_ARRAY(i), TAU_2_STD_ARRAY(i));
    fprintf('%s\n', dir_name);
end
fprintf('\nFiles per directory: Sample_001_flatrandom.mat through Sample_%03d_flatrandom.mat\n', config.N_images);
