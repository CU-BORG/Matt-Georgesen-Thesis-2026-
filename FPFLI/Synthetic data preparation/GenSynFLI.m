...
thres = 50; % lower intensity threshold 
h = 0.039;
...
function r = poissrnd(lambda)
    % Simple Poisson random number generator without Statistics Toolbox
    % Based on Knuth's algorithm for small lambda values
    ...
    if lambda < 30
        ...
        % Poisson(lambda) ≈ Normal(lambda, lambda) for large lambda
        r = round(lambda + sqrt(lambda) * randn());
        r = max(0, r);  % Ensure non-negative
    end
end
