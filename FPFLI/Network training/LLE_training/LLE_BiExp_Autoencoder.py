...
# -*- coding: utf-8 -*-
"""
LLE Bi-Exponential Autoencoder for FLIM Parameter Extraction
...
This autoencoder network extracts 3 parameters (tau1, tau2, f1) from FLIM decay curves
...
class LLE_BiExp_Autoencoder(nn.Module):
    ...
    latent_dim : int
        Latent space dimensionality (default: 3 for tau1, tau2, f1)
    """
    ...
    def __init__(self, dim=16, depth=8, kernel_size=9, patch_size=8,
        ...
        # =====================================================================
        # ENCODER: Compress decay + IRF → latent parameters
        # =====================================================================
        ...
        for _ in range(depth):
            self.encoder_blocks.append(ConvLayer(dim*2, kernel_size))

        # Compression to latent space
        # From [B, dim*2, 32] → [B, dim*2*32, 1] → [B, latent_dim]
        self.encoder_flatten = nn.Flatten()
        self.encoder_fc1 = nn.Linear(dim * 2 * self.encoded_length, dim * 4)
        ...
        self.decoder_fc2 = nn.Linear(dim * 4, dim * 2 * self.encoded_length)

        # Decoder convolutional blocks
        self.decoder_blocks = nn.ModuleList()
        for _ in range(depth):
            self.decoder_blocks.append(ConvLayer(dim*2, kernel_size))

        # Reconstruction layers
        # From [B, dim*2, 32] → [B, 1, 256]
        self.decoder_upsample = nn.ConvTranspose1d(
            dim*2, dim, kernel_size=patch_size, stride=patch_size
        )
    ...
    def encode(self, decay, irf):
        """
        Encode decay + IRF into 3-parameter latent space
        ...
        --------
        latent : Tensor [B, 3]
            Latent parameters [tau1, tau2, f1] (unconstrained)
        """
        # Embed inputs: [B, 1, 256] → [B, dim, 32]
        decay_emb = self.embd_decay(decay)
        irf_emb = self.embd_irf(irf)

        # Concatenate: [B, dim*2, 32]
        x = torch.cat([decay_emb, irf_emb], dim=1)
    ...
    def decode(self, latent):
        ...
        -----------
        latent : Tensor [B, 3]
            Latent parameters [tau1, tau2, f1]
        ...
        """
        # Expand from latent: [B, 3] → [B, dim*2*32]
        x = self.decoder_fc1(latent)
        x = self.relu(x)
        x = self.decoder_fc2(x)
        x = self.relu(x)

        # Reshape: [B, dim*2*32] → [B, dim*2, 32]
        batch_size = x.shape[0]
        x = x.view(batch_size, self.dim * 2, self.encoded_length)

        # Apply decoder blocks
        for block in self.decoder_blocks:
            x = block(x)

        # Upsample: [B, dim*2, 32] → [B, dim, 256]
    ...
    def forward(self, decay, irf):
        ...
        """
        latent = self.encode(decay, irf)
        reconstruction = self.decode(latent)
        return reconstruction, latent

    def extract_parameters(self, decay, irf):
        """
        ...
        --------
        dict with keys:
            - 'tau1': Tensor [B] - Short lifetime (constrained > 0)
            - 'tau2': Tensor [B] - Long lifetime (constrained > 0)
            - 'f1': Tensor [B] - Fraction of component 1 (constrained 0-1)
            - 'f2': Tensor [B] - Fraction of component 2 (= 1 - f1)
            - 'tau_avg': Tensor [B] - Average lifetime
        """
        latent = self.encode(decay, irf)

        # Apply constraints to latent parameters
        tau1 = self.relu(latent[:, 0])  # tau1 > 0
        tau2 = self.relu(latent[:, 1])  # tau2 > 0
        f1 = self.sigmoid(latent[:, 2])  # 0 < f1 < 1
        f2 = 1.0 - f1  # f2 = 1 - f1 (ensures sum to 1)

        # Compute average lifetime
        tau_avg = tau1 * f1 + tau2 * f2
    ...
    def forward_with_parameters(self, decay, irf):
        """
        ...
        --------
        reconstruction : Tensor [B, 1, 256]
        parameters : dict with tau1, tau2, f1, f2, tau_avg
        """
        reconstruction, latent = self.forward(decay, irf)

        # Apply constraints for parameter extraction
        tau1 = self.relu(latent[:, 0])
        tau2 = self.relu(latent[:, 1])
        f1 = self.sigmoid(latent[:, 2])
        f2 = 1.0 - f1
        tau_avg = tau1 * f1 + tau2 * f2
...
if __name__ == "__main__":
    print("=" * 80)
    print(" TESTING LLE_BiExp_Autoencoder")
    ...
    print(f"  Latent space: 3D [tau1, tau2, f1]")
    ...
    irf = torch.randn(batch_size, 1, 256)

    print(f"Test input shapes:")
    print(f"  decay: {decay.shape}")
    print(f"  irf: {irf.shape}")
    print()

    # Test encoding
    print("Testing encoder...")
    with torch.no_grad():
        latent = model.encode(decay, irf)
    ...
    with torch.no_grad():
        recon, latent = model.forward(decay, irf)
    ...
    with torch.no_grad():
        params = model.extract_parameters(decay, irf)

    print(f"\nExtracted parameters (first sample):")
    print(f"  tau1 = {params['tau1'][0].item():.4f} ns (constrained > 0)")
    print(f"  tau2 = {params['tau2'][0].item():.4f} ns (constrained > 0)")
    print(f"  f1 = {params['f1'][0].item():.4f} (constrained 0-1)")
    print(f"  f2 = {params['f2'][0].item():.4f} (= 1 - f1)")
    print(f"  tau_avg = {params['tau_avg'][0].item():.4f} ns")
    print()
    ...
    tau_avg_check = params['tau1'] * params['f1'] + params['tau2'] * params['f2']
    tau_avg_error = (tau_avg_check - params['tau_avg']).abs().mean()
    print(f"  [OK] tau_avg = tau1*f1 + tau2*f2: error = {tau_avg_error.item():.6e}")
    print()

    # Test combined forward
    print("Testing combined forward (for training)...")
    with torch.no_grad():
        recon, params = model.forward_with_parameters(decay, irf)
    print(f"  Reconstruction shape: {recon.shape}")
