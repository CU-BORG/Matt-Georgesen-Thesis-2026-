...
@author: Based on Visschers et al. 2021 + Training_LLE_BigSet.py template
"""
...
import glob
from LLE_BiExp_Autoencoder import LLE_BiExp_Autoencoder
...
def load_biexp_data_from_directory(data_root, test_ratio=0.2, BATCH_SIZE=128):
    ...
    for idx, mat_file in enumerate(mat_files):
        ...
        try:
            ...
            if 'tau_gt_components' in data:
                tau_components = data['tau_gt_components']  # [H, W, 2]
                f_components = data['f_gt_components']      # [H, W, 2]
            elif 'tau_components' in data:
                tau_components = data['tau_components']
                f_components = data['f_components']
            else:
                print(f"Warning: {mat_file} missing bi-exp components, skipping...")
                continue
...
if __name__ == '__main__':
    ...
    thesis_root = os.path.join(script_dir, '..', '..', '..')
    data_root = os.path.join(thesis_root, 'trainingbigset')
