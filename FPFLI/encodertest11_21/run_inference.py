#!/usr/bin/env python3
"""
Run inference on test .mat images using a trained spatialcorr model.
Loads 5 samples, extracts tau1/tau2/f per-pixel, compares to ground truth,
prints per-image stats and saves comparison plots.
...
# paths
# ---------------------------------------------------------------------------
MODEL_PATH = r'C:\Users\mcg11923\Thesis\training_logs_decay_bin62_12.5ns_20260306_112041\model_final_decay_bin62_12.5ns.pth'
DATA_DIR   = r'E:\decay_peak_bin62_12.5ns_train'
OUT_DIR    = r'C:\Users\mcg11923\Thesis\training_logs_decay_bin62_12.5ns_20260306_112041\inference_results'
N_IMAGES   = 5          # how many .mat files to run
...
# per-image inference
# ---------------------------------------------------------------------------
all_errors = {k: [] for k in ('tau1', 'tau2', 'f')}
...
for file_idx, mat_name in enumerate(mat_files):
    ...
    decays_list, gt_tau1_list, gt_tau2_list, gt_f_list = [], [], [], []
    ...
    gt_f    = np.array(gt_f_list,   dtype=np.float32)
    ...
    BATCH = 4096
    pred_tau1_list, pred_tau2_list, pred_f_list = [], [], []
    ...
    with torch.no_grad():
        for start in range(0, n_valid, BATCH):
            ...
            pred_f_list.append(params['f1'].cpu().numpy())
    ...
    all_gt_f.append(gt_f);         all_pred_f.append(pred_f)
    ...
    rows = [
        ...
        ('Bound Fraction' + r' ($f$)',   gt_f_img,   pred_f_img,   err_f_img,   stats_f),
    ]
    ...
    print(f"  time: {t_total:.1f}s  [load: {t_load*1000:.1f}ms | preprocess: {t_preprocess*1000:.1f}ms | inference: {t_inference*1000:.1f}ms | postprocess: {t_postprocess*1000:.1f}ms]\n")
...
gt_f_all     = np.concatenate(all_gt_f)
pred_f_all   = np.concatenate(all_pred_f)

total_pixels = len(gt_tau1_all)
print(f"\nCombined scatter: {total_pixels:,} pixels across {len(mat_files)} images")

fig, axes = plt.subplots(1, 4, figsize=(20, 5))
fig.suptitle('Predicted vs Ground Truth  —  all images combined  (decay_bin62_12.5ns)',
             fontsize=13, y=1.02)

panels = [
    (axes[0], gt_tau1_all,  pred_tau1_all, r'$\tau_1$' + ' (ns)', 'single'),
    (axes[1], gt_tau2_all,  pred_tau2_all, r'$\tau_2$' + ' (ns)', 'single'),
    (axes[2], gt_f_all,    pred_f_all,   r'Bound Fraction ($f$)', 'single'),
]

for ax, gt, pred, label, plot_type in panels:
...
# Fourth plot: Combined tau1 and tau2 predicted vs ground truth on same axis
ax = axes[3]
mae_tau1 = np.abs(pred_tau1_all - gt_tau1_all).mean()
mae_tau2 = np.abs(pred_tau2_all - gt_tau2_all).mean()
rmse_tau1 = np.sqrt(((pred_tau1_all - gt_tau1_all)**2).mean())
rmse_tau2 = np.sqrt(((pred_tau2_all - gt_tau2_all)**2).mean())

# Scatter plots with different colors for tau1 and tau2
ax.scatter(gt_tau1_all, pred_tau1_all, s=0.8, alpha=0.35, c='steelblue',
           edgecolors='none', label=r'$\tau_1$')
ax.scatter(gt_tau2_all, pred_tau2_all, s=0.8, alpha=0.35, c='green',
           edgecolors='none', label=r'$\tau_2$')

# Identity line limits from the combined data
lo = min(gt_tau1_all.min(), gt_tau2_all.min(), pred_tau1_all.min(), pred_tau2_all.min())
hi = max(gt_tau1_all.max(), gt_tau2_all.max(), pred_tau1_all.max(), pred_tau2_all.max())
pad = (hi - lo) * 0.03
lo -= pad; hi += pad
...
# Stats box for both tau1 and tau2
txt = (f"$\\tau_1$: $n$ = {len(gt_tau1_all):,}\n"
       f"       MAE  = {mae_tau1:.4f}\n"
       f"       RMSE = {rmse_tau1:.4f}\n\n"
       f"$\\tau_2$: $n$ = {len(gt_tau2_all):,}\n"
       f"       MAE  = {mae_tau2:.4f}\n"
       f"       RMSE = {rmse_tau2:.4f}")
ax.text(0.04, 0.96, txt, transform=ax.transAxes,
...
plt.tight_layout()
scatter_path = os.path.join(OUT_DIR, 'combined_predicted_vs_gt.png')
