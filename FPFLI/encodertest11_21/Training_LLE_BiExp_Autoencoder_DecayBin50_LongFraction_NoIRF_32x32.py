...
DIMENSION UPDATE: Uses 32x32 intermediate dimensions (32 feature channels × 32 time bins)
...
def train_stage1_reconstruction(train_set, test_set, model, learning_rate=1e-4,
    ...
    """
    print("\n" + "=" * 80)
    print(" STAGE 1: RECONSTRUCTION TRAINING (Unsupervised, Decay-Only, 32x32 Dims)")
    print("=" * 80)
...
def train_stage2_parameters(train_set, test_set, model, learning_rate=5e-5,
    ...
    """
    print("\n" + "=" * 80)
    print(" STAGE 2: PARAMETER LEARNING (Supervised, Decay-Only, 32x32 Dims)")
    print("=" * 80)
...
def train_stage3_joint(train_set, test_set, model, learning_rate=1e-5,
    ...
    """
    print("\n" + "=" * 80)
    print(" STAGE 3: JOINT OPTIMIZATION (Decay-Only, 32x32 Dims)")
...
# =============================================================================
# MAIN TRAINING SCRIPT - DECAY PEAK BIN 62 (DECAY-ONLY, NO IRF, 32x32 DIMS)
# =============================================================================
if __name__ == '__main__':
    print("\n" + "=" * 80)
    print(" THREE-STAGE AUTOENCODER TRAINING - DECAY PEAK BIN 62")
    print(" DECAY-ONLY INPUT (NO IRF)")
    print(" 32x32 INTERMEDIATE DIMENSIONS (32 feature channels × 32 time bins)")
    ...
    # =========================================================================
    # MODEL HYPERPARAMETERS  (32x32 dimensions)
    # =========================================================================
    dim   = 32  # CHANGED from 16 to 32 for 32x32 intermediate dimensions
    depth = 8
    ks    = 9
    ps    = 8
    ...
    print("IMPORTANT: Model has 32x32 intermediate dimensions\n")
    ...
    ckpt_dir  = f'./training_logs_decay_bin62_12.5ns_f2_longfrac_NoIRF_32x32_{timestamp}'
    ...
    with open(dataset_info_path, 'w') as f:
        f.write("DECAY PEAK BIN 62 DATASET INFO - 12.5ns CALIBRATION - DECAY-ONLY (NO IRF) - 32x32 DIMS\n")
        ...
        f.write("Intermediate dimensions: 32 feature channels × 32 time bins\n\n")
        ...
        f.write("  - f1 + f2 = 1.0\n")
        f.write("  - Input: DECAY-ONLY (no IRF)\n")
        f.write("  - Intermediate dimensions: 32 channels × 32 time bins\n\n")
        ...
        f.write("  - Higher model capacity (32 channels vs 16 channels)\n\n")
        ...
        f.write("  - Test effect of increased intermediate dimensions (32x32)\n")
    ...
    # MODEL
    # =========================================================================
    print("\nInitializing autoencoder model (DECAY-ONLY, 32x32 DIMS)...")
    ...
    with open(arch_path, 'w') as f:
        f.write("LLE Bi-Exponential Autoencoder Architecture - DECAY-ONLY (NO IRF) - 32x32 DIMS\n")
        f.write("TRAINING FOR LONG LIFETIME FRACTION (f2)\n")
        f.write("=" * 80 + "\n\n")
        f.write(f"Hyperparameters:\n")
        f.write(f"  dim (channel dimension): {dim} (32 feature channels)\n")
        ...
        f.write(f"  intermediate representation: [B, 32, 32] (32 channels × 32 time bins)\n\n")
        f.write(str(model))
    ...
    with open(param_count_path, 'w') as f:
        ...
        f.write(f"Input: DECAY-ONLY (no IRF)\n")
        f.write(f"Intermediate dimensions: 32 channels × 32 time bins\n")
    ...
    # ------------------------------------------------------------------
    print("\n" + "=" * 80)
    print(" STAGE 1: Learning to reconstruct decay signals (bin 62 timing, DECAY-ONLY, 32x32 DIMS)")
    print("=" * 80)
    ...
    torch.save({
        ...
        'intermediate_dims': '32x32',
        'note': 'Training for LONG lifetime (bound) fraction, DECAY-ONLY input (no IRF), 32x32 intermediate dimensions'
    ...
    # ------------------------------------------------------------------
    print("\n" + "=" * 80)
    print(" STAGE 2: Learning tau1, tau2, f2 (LONG/BOUND fraction) from bin 62 ground truth (DECAY-ONLY, 32x32 DIMS)")
    print("=" * 80)
    ...
    # ------------------------------------------------------------------
    print("\n" + "=" * 80)
    print(" STAGE 3: Joint optimisation (reconstruction + parameters, DECAY-ONLY, 32x32 DIMS)")
    print("=" * 80)
    ...
    torch.save({
        ...
        'data_info': {
            ...
            'description': 'Decay peak bin 62 - 12.5ns temporal calibration - Training for LONG lifetime (bound) fraction (f2) - DECAY-ONLY input (no IRF) - 32x32 intermediate dimensions'
        },
        'important_note': 'MODEL OUTPUTS f2 (LONG LIFETIME/BOUND FRACTION), NOT f1. f2 = fraction of tau2 component. INPUT: DECAY-ONLY (no IRF). INTERMEDIATE DIMS: 32x32',
        'info': 'Three-stage trained autoencoder on decay_peak_bin62_12.5ns_trainfixed - f2 (long lifetime) target - DECAY-ONLY - 32x32 DIMS (Visschers et al. 2021)'
    }, os.path.join(ckpt_dir, 'model_final_decay_bin62_12.5ns_f2_longfrac_NoIRF_32x32.pth'))
    ...
    print(f"\n{'=' * 80}")
    print(" TRAINING COMPLETED - DECAY PEAK BIN 62 (12.5ns CALIBRATION, DECAY-ONLY, 32x32 DIMS)")
    print(" TRAINING TARGET: f2 (LONG LIFETIME/BOUND FRACTION)")
    print(" INPUT: DECAY-ONLY (NO IRF)")
    print(" INTERMEDIATE DIMENSIONS: 32 feature channels × 32 time bins")
    print(f"{'=' * 80}")
    print(f"  Total time   : {total_time:.2f} s  ({total_time/60:.1f} min)")
    print(f"  Checkpoints  : {ckpt_dir}/")
    print(f"  Final model  : {ckpt_dir}/model_final_decay_bin62_12.5ns_f2_longfrac_NoIRF_32x32.pth")
    ...
    print(f"  IMPORTANT    : 32x32 intermediate dimensions (higher capacity)")
    print("=" * 80 + "\n")

    # Save final summary
    summary_path = os.path.join(ckpt_dir, 'training_summary.txt')
    with open(summary_path, 'w') as f:
        f.write("TRAINING SUMMARY - DECAY PEAK BIN 62 (12.5ns CALIBRATION, DECAY-ONLY, 32x32 DIMS)\n")
        ...
        f.write("  - f1 + f2 = 1.0\n")
        f.write("  - INPUT: DECAY-ONLY (no IRF)\n")
        f.write("  - INTERMEDIATE DIMS: 32 channels × 32 time bins\n\n")
        f.write("Model hyperparameters:\n")
        f.write(f"  dim: {dim} (32 feature channels for higher capacity)\n")
        f.write(f"  depth: {depth}\n")
        f.write(f"  kernel_size: {ks}\n")
        f.write(f"  patch_size: {ps}\n")
        f.write(f"  input: DECAY-ONLY (no IRF)\n")
        f.write(f"  intermediate representation: [B, 32, 32]\n")
        f.write(f"  total parameters: {total_params:,}\n")
        ...
        f.write("  Input: DECAY-ONLY (no IRF)\n\n")
        f.write("Model improvements over standard version:\n")
        f.write("  - 32 feature channels (vs 16) for higher model capacity\n")
        f.write("  - 32×32 intermediate representation (vs 16×32)\n")
        f.write("  - Approximately 4× more parameters\n")
        f.write("  - Potential for better feature learning and parameter accuracy\n\n")
        f.write("Next steps:\n")
        ...
        f.write("  - Compare with 16×32 model to assess capacity benefits\n")
