...
# -*- coding: utf-8 -*-
"""
Three-Stage Training for LLE Bi-Exponential Autoencoder - FULLY FLAT RANDOM

Single-model training run on the fully_flatrandom dataset:
  - 400 .mat files (256x256 pixels, 256 time-bins each)
  - Tau1: Uniform[0.05, 0.90] ns (NO spatial correlation)
  - Tau2: Uniform[1.20, 4.50] ns (NO spatial correlation)
  - Fraction f1: Uniform[0.1, 0.9] (NO spatial correlation)
  - Ground-truth tau1, tau2, f1 per pixel
...
# =============================================================================
# MAIN TRAINING SCRIPT - FULLY FLAT RANDOM (single run)
# =============================================================================
if __name__ == '__main__':
    print("\n" + "=" * 80)
    print(" THREE-STAGE AUTOENCODER TRAINING - FULLY FLAT RANDOM")
    print(" Following Visschers et al. 2021 methodology")
    print(" Dataset : fully_flatrandom (400 files, 256x256, 256 bins)")
    print(" Tau1    : Uniform[0.05, 0.90] ns (NO spatial correlation)")
    ...
    # =========================================================================
    data_root = (
        r'C:\Users\mcg11923\Thesis\FPFLI\Synthetic data preparation'
    ...
    if not os.path.exists(data_root):
        ...
        print(f"Run Generate_MultiExp_Image_Training_Data_FullyFlatRandom.m first to create the dataset.")
        sys.exit(1)

    # =========================================================================
    # LOAD DATA
    # =========================================================================
    print("\nLoading fully_flatrandom FLIM data...")
    ...
    ckpt_dir  = f'./training_logs_fully_flatrandom_{timestamp}'
    ...
    with open(dataset_info_path, 'w') as f:
        f.write("FULLY FLAT RANDOM DATASET INFO\n")
        f.write("=" * 80 + "\n\n")
        f.write("Dataset: fully_flatrandom\n")
        f.write("Total images: 400 (256x256 pixels, 256 time-bins)\n\n")
        f.write("Parameter Distributions (all UNIFORM, NO spatial correlation):\n")
        f.write("  Tau1: Uniform[0.05, 0.90] ns\n")
        ...
        f.write("  - Wider ranges than real data statistics\n")
        f.write("  - Maximum parameter space coverage\n")
    ...
    torch.save({
        ...
        'data_info': {
            'tau1_range': [0.05, 0.90],
            'tau2_range': [1.20, 4.50],
            'f1_range': [0.1, 0.9],
            'distribution': 'uniform',
            'spatial_correlation': False,
            'description': 'Fully flat random - all parameters uniform, no spatial correlation'
        },
        'info': 'Three-stage trained autoencoder on fully_flatrandom dataset (Visschers et al. 2021)'
    }, os.path.join(ckpt_dir, 'model_final_fully_flatrandom.pth'))
    ...
    print(f"\n{'=' * 80}")
    print(" TRAINING COMPLETED - FULLY FLAT RANDOM")
    print(f"{'=' * 80}")
    print(f"  Total time   : {total_time:.2f} s  ({total_time/60:.1f} min)")
    print(f"  Checkpoints  : {ckpt_dir}/")
    print(f"  Final model  : {ckpt_dir}/model_final_fully_flatrandom.pth")
    ...
    with open(summary_path, 'w') as f:
        f.write("TRAINING SUMMARY - FULLY FLAT RANDOM\n")
        f.write("=" * 80 + "\n\n")
        f.write(f"Dataset: fully_flatrandom\n")
