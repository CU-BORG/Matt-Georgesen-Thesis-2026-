"""
Run inference on real FLIM data with PHOTON-STRATIFIED ANALYSIS and ERROR CORRELATION MATRICES.
...
Additionally analyzes how prediction errors in different parameters (tau1, tau2, f)
are correlated within each photon bin to understand if error coupling varies with
signal strength.
...
# ============================================================================
# Dataset for Real FLIM Data (NO FILTERING - need all photon ranges)
# ============================================================================

class RealFLIMDataset(Dataset):
    """
    Dataset for real FLIM data - loads ALL valid pixels (no photon filtering).
    ...
    def _load_sample(self, mat_file):
        """Load a single .mat file and extract ALL valid pixels (no photon filtering)"""
        try:
...
def run_inference_on_real_data(model, dataloader, device):
    """
    Run inference on real FLIM data and collect model predictions vs Double Exponential Fit results.
...
# ============================================================================
# Error Correlation Analysis Functions
# ============================================================================

def compute_error_correlations(bin_results):
    """
    Compute error correlation matrix for a single photon bin.

    Args:
        bin_results: dict with predictions and ground truth for a bin

    Returns:
        dict with correlation matrix and individual correlation stats
    """
    # Compute errors
    err_tau1 = bin_results['tau1_pred'] - bin_results['tau1_gt']
    err_tau2 = bin_results['tau2_pred'] - bin_results['tau2_gt']
    err_f = bin_results['f_pred'] - bin_results['f_gt']

    # Create error array for correlation matrix
    errors = np.column_stack([err_tau1, err_tau2, err_f])

    # Compute Pearson correlation matrix
    corr_matrix = np.corrcoef(errors.T)

    # Compute individual correlations with p-values
    corr_tau1_tau2 = stats.pearsonr(err_tau1, err_tau2)
    corr_tau1_f = stats.pearsonr(err_tau1, err_f)
    corr_tau2_f = stats.pearsonr(err_tau2, err_f)

    return {
        'corr_matrix': corr_matrix,
        'correlations': {
            'tau1_tau2': {'r': corr_tau1_tau2[0], 'p': corr_tau1_tau2[1]},
            'tau1_f': {'r': corr_tau1_f[0], 'p': corr_tau1_f[1]},
            'tau2_f': {'r': corr_tau2_f[0], 'p': corr_tau2_f[1]}
        },
        'mae': {
            'tau1': np.abs(err_tau1).mean(),
            'tau2': np.abs(err_tau2).mean(),
            'f': np.abs(err_f).mean()
        }
    }


def plot_error_correlation_matrix(bin_results, bin_name, group_name, save_dir):
    """
    Plot error correlation matrix for a single photon bin.

    Args:
        bin_results: dict with predictions and ground truth for a bin
        bin_name: name of the photon bin (e.g., '200-400')
        group_name: name of the group (e.g., 'Control_Group')
        save_dir: directory to save plots
    """
    # Compute correlations
    corr_stats = compute_error_correlations(bin_results)
    corr_matrix = corr_stats['corr_matrix']
    ...
    sns.heatmap(corr_matrix, annot=True, fmt='.3f', cmap='RdBu_r',
                ...
                xticklabels=[r'$\tau_1$ error', r'$\tau_2$ error', r'$f$ error'],
                yticklabels=[r'$\tau_1$ error', r'$\tau_2$ error', r'$f$ error'],
                cbar_kws={'label': 'Pearson r'}, ax=ax)

    ax.set_title(f'{group_name} - {bin_name} photons\nError Correlation Matrix (n={bin_results["n_pixels"]:,})',
                 fontsize=12, fontweight='bold')

    plt.tight_layout()

    # Save
    save_path = os.path.join(save_dir, f'{group_name}_{bin_name}_correlation.png')
...
def plot_combined_correlation_matrices(stratified_results, group_name, save_dir):
    """
    Create combined multi-panel plot showing correlation matrices for all photon bins.
    ...
    Args:
        ...
        save_dir: directory to save plots
    """
    bin_names = sorted(stratified_results.keys(), key=lambda x: int(x.split('-')[0]))
    ...
    for idx, bin_name in enumerate(bin_names):
        ...
        sns.heatmap(corr_matrix, annot=True, fmt='.3f', cmap='RdBu_r',
                    center=0, vmin=-1, vmax=1, square=True,
                    xticklabels=[r'$\tau_1$', r'$\tau_2$', r'$f$'],
                    yticklabels=[r'$\tau_1$', r'$\tau_2$', r'$f$'],
                    cbar_kws={'label': 'Pearson r'} if idx == n_bins-1 else None,
                    cbar=idx == n_bins-1,
                    ax=ax)

        ax.set_title(f'{bin_name} photons\n(n={bin_results["n_pixels"]:,})',
                     fontsize=11, fontweight='bold')

    plt.suptitle(f'{group_name} - Error Correlation Matrices Across Photon Bins',
                 fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()

    # Save
    save_path = os.path.join(save_dir, f'{group_name}_all_bins_correlations.png')
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
...
def plot_stratified_predictions(stratified_results, group_name, save_dir):
    ...
    plt.suptitle(f'Model Prediction vs Double Exponential Fit Results - {group_name}\n(Photon-Stratified Analysis)', fontsize=14, fontweight='bold')
    plt.tight_layout()

    save_path = os.path.join(save_dir, f'{group_name}_photon_stratified.png')
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
...
def plot_mean_std_comparison(stratified_results, group_name, save_dir):
    ...
    ax.bar(x + width/2, tau1_model_means, width, yerr=tau1_model_stds,
           label='Model', color='coral', capsize=5, alpha=0.8)
    ax.set_ylabel(r'$\tau_1$ (ns)', fontsize=12, fontweight='bold')
    ax.set_title(f'{group_name} - Mean ± Std Comparison', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    save_path = os.path.join(save_dir, f'{group_name}_mean_std_comparison.png')
    plt.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.close()
    ...
    ax.set_ylabel(r'$\tau_1$ (ns)', fontsize=12, fontweight='bold')
    ax.set_xlabel('Photon Bin', fontsize=12, fontweight='bold')
    ax.set_title(f'{group_name} - ' + r'$\tau_1$ Mean ± Std Comparison', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ax.set_xticklabels(bin_names, fontsize=10)
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3, axis='y')
    plt.tight_layout()
    individual_path = os.path.join(individual_dir, f'{group_name}_tau1_mean_std.png')
    ...
    ax.set_ylabel(r'$\tau_2$ (ns)', fontsize=12, fontweight='bold')
    ax.set_xlabel('Photon Bin', fontsize=12, fontweight='bold')
    ax.set_title(f'{group_name} - ' + r'$\tau_2$ Mean ± Std Comparison', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ax.set_xticklabels(bin_names, fontsize=10)
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3, axis='y')
    plt.tight_layout()
    individual_path = os.path.join(individual_dir, f'{group_name}_tau2_mean_std.png')
    plt.savefig(individual_path, dpi=150, bbox_inches='tight')
    plt.close()
    ...
    ax.set_ylabel('f', fontsize=12, fontweight='bold')
    ax.set_xlabel('Photon Bin', fontsize=12, fontweight='bold')
    ax.set_title(f'{group_name} - f Mean ± Std Comparison', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ax.set_xticklabels(bin_names, fontsize=10)
    ax.legend(fontsize=11)
    ax.grid(True, alpha=0.3, axis='y')
    plt.tight_layout()
    individual_path = os.path.join(individual_dir, f'{group_name}_f_mean_std.png')
    plt.savefig(individual_path, dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == '__main__':
    # Configuration
    MODEL_PATH = r'C:\Users\mcg11923\Thesis\training_logs_decay_bin62_12.5ns_f_fixed_20260407_103648\model_final_decay_bin62_12.5ns_f2_longfrac.pth'
    REAL_DATA_DIR = r'E:\RealdataSCC74A\mat_files'
    OUT_DIR = r'C:\Users\mcg11923\Thesis\training_logs_decay_bin62_12.5ns_f_fixed_20260407_103648\real_data_photon_stratified_with_correlation'
    ...
    if not os.path.exists(MODEL_PATH):
        ...
        print("Please update MODEL_PATH to point to the trained model checkpoint.")
        sys.exit(1)
    ...
    print("=" * 80)
    print("PHOTON-STRATIFIED INFERENCE WITH ERROR CORRELATION ANALYSIS")
    print("=" * 80)
    ...
    for group in groups:
        ...
        # NEW: Generate error correlation matrices
        print("Generating error correlation matrices...")

        # Create correlation subdirectory
        corr_dir = os.path.join(OUT_DIR, 'error_correlations')
        os.makedirs(corr_dir, exist_ok=True)

        # Plot individual correlation matrices for each bin
        for bin_name in sorted(stratified_results.keys(), key=lambda x: int(x.split('-')[0])):
            corr_path = plot_error_correlation_matrix(
                stratified_results[bin_name], bin_name, display_name, corr_dir
            )
            print(f"  {bin_name} correlation matrix: {corr_path}")

        # Plot combined correlation matrices
        combined_path = plot_combined_correlation_matrices(stratified_results, display_name, corr_dir)
        print(f"Combined correlation matrices: {combined_path}\n")

        # Compute correlation statistics for summary
        bin_correlations = {}
        for bin_name in sorted(stratified_results.keys(), key=lambda x: int(x.split('-')[0])):
            corr_stats = compute_error_correlations(stratified_results[bin_name])
            bin_correlations[bin_name] = corr_stats
    ...
    # Save enhanced summary with correlation statistics
    summary_path = os.path.join(OUT_DIR, 'photon_stratified_summary_with_correlations.txt')
    with open(summary_path, 'w', encoding='utf-8') as f:
        f.write("=" * 80 + "\n")
        f.write("PHOTON-STRATIFIED INFERENCE WITH ERROR CORRELATION ANALYSIS\n")
        f.write("=" * 80 + "\n\n")
        ...
        for group, group_data in all_group_results.items():
            ...
            f.write("ERROR CORRELATIONS BY PHOTON BIN:\n")
            f.write("-" * 70 + "\n")
            for bin_name in sorted(group_data['bin_correlations'].keys(),
                                   key=lambda x: int(x.split('-')[0])):
                corr_stats = group_data['bin_correlations'][bin_name]
                corrs = corr_stats['correlations']
                f.write(f"\n{bin_name} photons:\n")
                f.write(f"  τ1 vs τ2:  r = {corrs['tau1_tau2']['r']:7.4f}  (p = {corrs['tau1_tau2']['p']:.2e})\n")
                f.write(f"  τ1 vs f:   r = {corrs['tau1_f']['r']:7.4f}  (p = {corrs['tau1_f']['p']:.2e})\n")
                f.write(f"  τ2 vs f:   r = {corrs['tau2_f']['r']:7.4f}  (p = {corrs['tau2_f']['p']:.2e})\n")
            f.write("\n")

    print("=" * 80)
    print("PHOTON-STRATIFIED ANALYSIS WITH ERROR CORRELATION COMPLETE")
    print("=" * 80)
    print(f"Summary saved: {summary_path}")
    print(f"All plots saved to: {OUT_DIR}")
    print(f"Correlation matrices saved to: {os.path.join(OUT_DIR, 'error_correlations')}")
    print("\nPhoton-stratified inference with correlation analysis complete!")
