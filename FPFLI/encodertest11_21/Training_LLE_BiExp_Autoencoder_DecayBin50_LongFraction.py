...
TRAINING FOR LONG LIFETIME FRACTION (f2)
...
# =============================================================================
# CUSTOM DATA LOADER - EXTRACTS f2 (LONG LIFETIME FRACTION) INSTEAD OF f1
# =============================================================================
def load_biexp_data_f2_from_directory(data_root, test_ratio=0.2, BATCH_SIZE=128):
    """
    Load bi-exponential FLIM data from directory - EXTRACTS f2 (LONG LIFETIME FRACTION)

    This is a modified version that extracts f2 (long lifetime/bound fraction)
    instead of f1 (short lifetime fraction).
    ...
    Returns:
    --------
    train_set, test_set: DataLoaders with (decay, irf, tau1, tau2, f2)
    """
    ...
    print(f"Parameter ranges (TRAINING FOR LONG LIFETIME FRACTION f2):")
    print(f"  tau1 (short): [{all_tau1.min():.4f}, {all_tau1.max():.4f}] ns")
    print(f"  tau2 (long):  [{all_tau2.min():.4f}, {all_tau2.max():.4f}] ns")
    print(f"  f2 (LONG/BOUND fraction): [{all_f2.min():.4f}, {all_f2.max():.4f}]")  # CHANGED: labeling
...
# =============================================================================
# MAIN TRAINING SCRIPT - DECAY PEAK BIN 62 (MATCHED TO REAL DATA)
# =============================================================================
if __name__ == '__main__':
    ...
    # =========================================================================
    # LOAD DATA - USING CUSTOM LOADER FOR f2 (LONG LIFETIME FRACTION)
    # =========================================================================
    ...
    train_set, test_set = load_biexp_data_f2_from_directory(
    ...
    ckpt_dir  = f'./training_logs_decay_bin62_12.5ns_f2_longfrac_{timestamp}'
    ...
    with open(dataset_info_path, 'w') as f:
        f.write("DECAY PEAK BIN 62 DATASET INFO - 12.5ns CALIBRATION\n")
    ...
    # ------------------------------------------------------------------
    print("\n" + "=" * 80)
    print(" STAGE 1: Learning to reconstruct decay signals (bin 62 timing)")
    print("=" * 80)
    ...
    torch.save({
        ...
        'note': 'Training for LONG lifetime (bound) fraction, not short lifetime fraction'
    ...
    # ------------------------------------------------------------------
    print("\n" + "=" * 80)
    print(" STAGE 2: Learning tau1, tau2, f2 (LONG/BOUND fraction) from bin 62 ground truth")
    print("=" * 80)
    ...
    torch.save({
        ...
        'data_info': {
            ...
            'description': 'Decay peak bin 62 - 12.5ns temporal calibration - Training for LONG lifetime (bound) fraction (f2)'
        },
        'important_note': 'MODEL OUTPUTS f2 (LONG LIFETIME/BOUND FRACTION), NOT f1. f2 = fraction of tau2 component.',
        'info': 'Three-stage trained autoencoder on decay_peak_bin62_12.5ns_trainfixed - f2 (long lifetime) target (Visschers et al. 2021)'
    }, os.path.join(ckpt_dir, 'model_final_decay_bin62_12.5ns_f2_longfrac.pth'))
    ...
    print(f"\n{'=' * 80}")
    print(" TRAINING COMPLETED - DECAY PEAK BIN 62 (12.5ns CALIBRATION)")
    print(" TRAINING TARGET: f2 (LONG LIFETIME/BOUND FRACTION)")
    print(f"{'=' * 80}")
    print(f"  Total time   : {total_time:.2f} s  ({total_time/60:.1f} min)")
    print(f"  Checkpoints  : {ckpt_dir}/")
    print(f"  Final model  : {ckpt_dir}/model_final_decay_bin62_12.5ns_f2_longfrac.pth")
    ...
    with open(summary_path, 'w') as f:
        f.write("TRAINING SUMMARY - DECAY PEAK BIN 62 (12.5ns CALIBRATION)\n")
