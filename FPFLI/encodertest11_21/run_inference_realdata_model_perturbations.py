#!/usr/bin/env python3
"""
Run the realdata_stats trained model on all perturbation groups + baseline.
Produces per-image GT/Pred/Error maps, a combined scatter per group,
and a final cross-group summary bar-chart.

Adapted from run_inference_all_perturbations.py to use the realdata_stats model.
...
# ---------------------------------------------------------------------------
# CONFIG - SPECIFIED MODEL PATH
# ---------------------------------------------------------------------------
# Using the specific trained model from training_logs_fraction_weighted_level03_20260202_174238
MODEL_PATH = r'C:\Users\mcg11923\Thesis\FPFLI\training_logs_fraction_weighted_level03_20260202_174238\model_final_fraction_weighted_level03.pth'

DATA_BASE  = (r'C:\Users\mcg11923\Thesis\FPFLI\Synthetic data preparation'
              r'\generate_multiexp_spectrumless\generate_spectrum_lite')

OUT_BASE   = (r'C:\Users\mcg11923\Thesis\FPFLI\training_logs_fraction_weighted_level03_20260202_174238'
...
N_IMAGES    = 10         # number of .mat files per group to process
SAMPLE_STEP = 1          # pixel subsampling (1 = every pixel, 4 = every 4th)
...
# ---------------------------------------------------------------------------
print("=" * 70)
print("  INFERENCE: REALDATA_STATS MODEL ON PERTURBATIONS")
print("=" * 70)
print(f"\nLoading model from:\n  {MODEL_PATH}")
...
if not os.path.exists(MODEL_PATH):
    ...
    print(f"Expected pattern: training_logs_realdata_stats_<timestamp>/model_final_realdata_stats.pth")
    sys.exit(1)
...
def run_one_image(mat_path):
    ...
    # IRF from bright-corner average
    ...
    # collect valid pixels (above intensity threshold)
...
def save_perimage_plot(res, mat_name, group_name, out_dir, file_idx):
    ...
    fig.suptitle(f'{mat_name}  ({group_name}, realdata_stats model)',
                 fontsize=14, y=0.98)
...
def save_group_scatter(all_gt, all_pred, group_name, out_dir):
    ...
    fig.suptitle(f'Predicted vs Ground Truth  —  {group_name}  (realdata_stats model)',
                 fontsize=13, y=1.02)
...
def save_summary_bar(summary, out_dir):
    ...
    fig.suptitle('Mean MAE by Perturbation Group  —  RealData Stats Model',
...
for group_name in GROUPS:
    ...
    if not os.path.exists(data_dir):
        print(f"\n{'='*70}")
        print(f"  WARNING: {group_name} directory not found, skipping")
        print(f"  {data_dir}")
        print(f"{'='*70}")
        continue
