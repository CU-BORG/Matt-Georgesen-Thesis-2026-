...
class LLE_BiExp_Autoencoder(nn.Module):
    ...
    (Decay-Only Input Version with 32x32 Intermediate Dimensions)
    ...
    Architecture:
        Input: Decay [B, 1, 256]
          ↓
        Encoder: ConvMixer layers → [B, 32, 32] → 3D latent space
          ↓
        Latent: [tau1, tau2, f]
          ↓
        Decoder: [B, 32, 32] → Reconstruct decay curve
          ↓
    ...
    -----------
    dim : int
        Feature dimension for ConvMixer (default: 32, creates 32x32 intermediate representation)
    depth : int
    ...
    def __init__(self, dim=32, depth=8, kernel_size=9, patch_size=8,
        ...
        # =====================================================================
        # ENCODER: Compress decay → latent parameters top half of model
        # =====================================================================
        ...
        # =====================================================================
        # DECODER: Reconstruct decay from latent parameters bottom half of model
        # =====================================================================
    ...
    def decode(self, latent):
        ...
        """
        # Expand from latent: [B, 3] → [B, 32*32]
        x = self.decoder_fc1(latent)
        x = self.relu(x)
        x = self.decoder_fc2(x)
        x = self.relu(x)

        # Reshape: [B, 32*32] → [B, 32, 32]
        ...
        # Upsample: [B, 32, 32] → [B, 32, 256]
        x = self.decoder_upsample(x)

        # Final output: [B, 32, 256] → [B, 1, 256]
    ...
    def extract_parameters(self, decay):
        ...
        dict with keys:
            ...
            - 'f2': Tensor [B] - Fraction of tau2 (long/bound component, constrained 0-1)
            - 'f1': Tensor [B] - Fraction of tau1 (short/free component, = 1 - f2)
            - 'tau_avg': Tensor [B] - Average lifetime = tau2*f2 + tau1*f1
        """
        latent = self.encode(decay)
        ...
        f2 = self.sigmoid(latent[:, 2])  # 0 < f2 < 1 (fraction of tau2/long)
        f1 = 1.0 - f2  # f1 = fraction of tau1/short (ensures sum to 1)

        # Compute average lifetime
        tau_avg = tau2 * f2 + tau1 * f1
    ...
    def forward_with_parameters(self, decay):
        ...
        parameters : dict with:
            ...
            - f2: fraction of tau2 (long/bound component)
            - f1: fraction of tau1 (short/free component)
            - tau_avg: average lifetime = tau2*f2 + tau1*f1
        """
        ...
        f2 = self.sigmoid(latent[:, 2])  # fraction of tau2/long
        f1 = 1.0 - f2  # fraction of tau1/short
        tau_avg = tau2 * f2 + tau1 * f1
...
if __name__ == "__main__":
    print("=" * 80)
    print(" TESTING LLE_BiExp_Autoencoder (Decay-Only Input, 32x32 Dimensions)")
    ...
    print(f"  Intermediate: 32 channels × 32 time bins")
    ...
    print(f"  f2 = {params['f2'][0].item():.4f} (fraction of tau2/long, constrained 0-1)")
    print(f"  f1 = {params['f1'][0].item():.4f} (fraction of tau1/short, = 1 - f2)")
    print(f"  tau_avg = {params['tau_avg'][0].item():.4f} ns (= tau2*f2 + tau1*f1)")
    print()
    ...
    print(f"  [OK] 0 < f2 < 1: range = [{params['f2'].min().item():.4f}, {params['f2'].max().item():.4f}]")

    f_sum = params['f1'] + params['f2']
    print(f"  [OK] f1 + f2 = 1: mean = {f_sum.mean().item():.6f}, std = {f_sum.std().item():.6e}")

    tau_avg_check = params['tau2'] * params['f2'] + params['tau1'] * params['f1']
    tau_avg_error = (tau_avg_check - params['tau_avg']).abs().mean()
    print(f"  [OK] tau_avg = tau2*f2 + tau1*f1: error = {tau_avg_error.item():.6e}")
    print()
    ...
    print("  [B, 32, 32]  ← 32 feature channels × 32 time bins")
